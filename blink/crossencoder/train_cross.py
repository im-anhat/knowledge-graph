...
def main(params):
    ...
    candidate_input = train_data["candidate_vecs"] # nhat (# mentions, 64, 128)
    label_input = train_data["labels"]

    # nhat
    gold_entity_ids = train_data["labels_idx"]
    candidate_prime = train_data["candidates_prime"]
    
    # If using generated definitions, replace gold entity with prime candidate
    if params.get("setting") == "generated_def":
        logger.info("Using generated definitions: replacing gold entities with prime candidates")
        for i in range(len(candidate_input)):
            gold_pos = label_input[i].item()  # Position of gold entity in candidates
            logger.info(f"Sample {i}: Replacing candidate at position {gold_pos} with prime candidate")
            candidate_input[i][gold_pos] = candidate_prime[i]  # Replace with prime version
        logger.info(f"Replaced {len(candidate_input)} gold entities with prime candidates")
    # nhat
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            batch = tuple(t.to(device) for t in batch)
            context_input = batch[0] # nhat: (batch size, 64, 255)
            label_input = batch[1]
    ...
    logger.info("Best model saved at: {}".format(params["path_to_model"]))
...
if __name__ == "__main__":
    ...
    # Add setting parameter for generated definitions
    parser.add_argument(
        "--setting", 
        type=str, 
        default="original_def",
        choices=["original_def", "generated_def"],
        help="Whether to use original or generated entity definitions"
    )
