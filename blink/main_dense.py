...
def _load_candidates(
    ...
    with open(entity_catalogue, "r") as fin:
        ...
        for line in lines:
            ...
            ents_ids.append(wikipedia_id)
    local_idx = min(ents_ids)
    ...
    while not stopping_condition:
        ...
        if args.interactive:
            ...
            if not keep_all:
                ...
                # get biencoder prediction
                
                # for bid  in bi_e_id:
                #     bi_e_title = id2title[bid]
                #     bi_pred.append({'id':bid,  'title':bi_e_title})
            
                for entity_list in nns:
                    sample_prediction = []
                    for e_id in entity_list:
                        e_title = id2title[e_id]
                        sample_prediction.append({'id':e_id, 'title':e_title})
                    bi_pred.append(sample_prediction)
        ...
        # Dont remove examples where the gold entity is not among the candidates
        is_empty = candidate_input.numel() == 0
        if is_empty:
            context_input, candidate_input, label_input = prepare_crossencoder_data(
            crossencoder.tokenizer, samples, labels, nns, id2title, id2text, True,
        )
        ...
        # crossencoder_params["eval_batch_size"] = 1
        ...
        if args.interactive:
            ...
            for entity_list, index_list, scores_list in zip(
                ...
                for index in index_list:
                    ...
                    e_title = id2title[e_id]
                    sample_prediction.append({'id':e_id, 'title':e_title, 'score':scores_list[index]})
